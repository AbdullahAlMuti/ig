...
import { calculatePerformanceRankings } from '../src/shared/utils/performanceRanker';
...
describe('performanceRanker', () => {
  ...
  it('evaluates performance rankings across dataset', () => {
    const res = calculatePerformanceRankings(mockItems);
    expect(res.totalCount).toBe(10);
    expect(res.byCode.size).toBe(10);

    const post3 = res.byCode.get('post3');
    expect(post3?.overallRank).toBe(1);
    expect(post3?.badge?.type).toBe('top-performer');
  });

  it('assigns selective badges correctly', () => {
    const res = calculatePerformanceRankings(mockItems);
    const post3 = res.byCode.get('post3');
    expect(post3?.badge?.type).toBe('top-performer');

    const post2 = res.byCode.get('post2');
    expect(post2?.badge?.type).toBe('most-engaged');

    const post4 = res.byCode.get('post4');
    expect(post4?.badge).toBeNull();
  });
});
